...
import {
  ArticleSlice,
  NegativeArticleSlice,
  PietotalnegativeresultSlice,
  PietotalpositiveresultSlice,
...
export const ArticledisplayAction =
  (name, sort = "desc", orderField = "published") =>
  async (dispatch) => {
    try {
      ...
      const fetchData = async (query) => {
        try {
          const response = await axios.get(
            `${BASE_URL}/newsApiLite?token=${token}&q=${query}`,
            config
          );
...
// Pagination
export const NexpaginationAction =
  (positiveUrl, negativeUrl) => async (dispatch) => {
    try {
      const config = {
        headers: {
          "Content-Type": "application/json",
      ...
      const fetchData = async () => {
        try {
          // Fetch positive articles
          const positiveResponse = await axios.get(
            `${BASE_URL}/${positiveUrl}`,
            config
          );
          dispatch(ArticleSlice(positiveResponse.data));

          // Fetch negative articles
          const negativeResponse = await axios.get(
            `${BASE_URL}/${negativeUrl}`,
            config
          );
