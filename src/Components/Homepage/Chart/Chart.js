...
export default function Chart() {
  ...
  const pietotalPositiveCount = useSelector(
    (state) => state.news.pietotalPositiveCount
  );
  const pietotalNegativeCount = useSelector(
    (state) => state.news.pietotalNegativeCount
  );
  const [isLoading, setIsLoading] = useState(true);
  const [tokenSubmitted, setTokenSubmitted] = useState(
    localStorage.getItem("tokenSubmitted")
  );
  ...
  if (isLoading) {
    ...
    chartContent = (
      <PieChart
        totalPositiveCount={randomPositiveCount}
        totalNegativeCount={randomNegativeCount}
        context="home"
      />
    );
  } else {
    chartContent = (
      <PieChart
        totalPositiveCount={pietotalPositiveCount}
        totalNegativeCount={pietotalNegativeCount}
        context="home"
      />
    );
  }
        ...
        <div className="row">
          ...
          <div className="col-lg-6 section-title pe-xxl-1">
            ...
            <div className="buttons-holder text-center text-lg-start">
              <a
                href="https://webz.io/products/news-api#lite"
                target="_blank"
